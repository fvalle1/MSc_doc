...
The output of this kind of model consists of sets of genes, the topics, with a probability distribution $P(\text{gene} | \text{topic})$ and probability distributions of these topics inside each sample $P(\text{topic} | \text{sample})$, both gives the relation between a \textit{sample} and a \textit{gene}.

In this work it is used an innovative and recent approach to topic modelling, the algorithm was presented by~\cite{gerlach2018network} and well explained in details in~\cite{Peixoto2017}, this belongs to the so-called stochastic block models~\cite{Holland1983}. 
...
\begin{figure}[htb!]
  ...
  \caption{Example of a hierarchical structure.}
