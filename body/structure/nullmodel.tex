...
\end{equation}
where $n_i$ is the number of components with frequency $f_i$, being $f_i=\frac{a_i}{\sum_{c=1}^{N}a_{c}}$ as defined in~\ref{eq:fi}.
...
\end{minipage}
\caption{Zipf's law sampled; TCGA(left) and GTEx (right). Sampling is reported for comparison.}
\label{fig:structure/globalzipf_null}
\end{figure}
By construction the distribution of the sizes of the sampling and of the data are alsoidentical.
\begin{figure}[htb!]
\begin{minipage}{0.5\textwidth}
    \centering
...
Plotting on the abscissa the size of samples, on the ordinate the number of genes expressed one point per sample, one obtains the so-called Heaps's law~\cite{Heaps:1978:IRC:539986}. In figure~\ref{fig:structure/heaps_null} the Heaps' law is presented compared to the one obtained by sampling. Again the curves differ and the null model is not enough complete to explain the trend. Note that each data point share the abscissa with a sampling one (figures~\ref{fig:structure/sizeDistr_null} are nothing but the histograms of the abscissas of~\ref{fig:structure/heaps_null}). Moreover it is interesting that the ordinate does not start from zero, this happens because there are lot of genes that express everywhere, the core. It happens that the sampling curve is above the data's one. This means that to build a sample of size $M$ just by sampling it is necessary to use a greater number of different genes than the number of different genes actually expressed in nature. In other words in real world are expressed only the genes that are really useful in the sample, and this is not derivable just by sampling. This fact is coherent with the fact that the $U$s differ.
\begin{figure}[htb!]
\begin{minipage}{0.5\textwidth}
    \centering
...
\begin{minipage}{0.5\textwidth}
    ...
    \end{minipage}
\caption{Number of genes expressed in a sample; TCGA(left) and GTEx (right). Sampling is reported for comparison.}
